...
namespace Engine {
	...
	void VulkanSwapChain::InitSurface(GLFWwindow* windowHandle)
	{
		...
		ENGINE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		ENGINE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync)
	{
		...
		ENGINE_ASSERT(presentModeCount > 0);
		...
		for (uint32_t i = 0; i < m_ImageCount; i++)
		{
			...
			SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_IMAGE_VIEW, fmt::format("Swapchain ImageView: {}", i), m_Images[i].ImageView);
		}
		...
		if (!m_Semaphores.RenderComplete || !m_Semaphores.PresentComplete)
		{
			...
			SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_SEMAPHORE, "Swapchain Semaphore RenderComplete", m_Semaphores.RenderComplete);
			VK_CHECK_RESULT(vkCreateSemaphore(m_Device->GetVulkanDevice(), &semaphoreCreateInfo, nullptr, &m_Semaphores.PresentComplete));
			SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_SEMAPHORE, "Swapchain Semaphore PresentComplete", m_Semaphores.PresentComplete);
		}
		...
		if (m_WaitFences.size() != m_ImageCount)
		{
			...
			for (auto& fence : m_WaitFences)
			{
				...
				SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_FENCE, "Swapchain Fence", fence);
			}
		}
		...
		SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_RENDER_PASS, "Swapchain render pass", m_RenderPass);
		...
		// Create framebuffers for every swapchain image
		{
			...
			for (uint32_t i = 0; i < m_Framebuffers.size(); i++)
			{
				...
				SetDebugUtilsObjectName(m_Device->GetVulkanDevice(), VK_OBJECT_TYPE_FRAMEBUFFER, fmt::format("Swapchain framebuffer (Frame in flight: {})", i), m_Framebuffers[i]);
			}
		}
	}
	...
	{
		LOG_ENGINE_WARN_TAG("Renderer", "VulkanSwapChain::OnDestroy");
