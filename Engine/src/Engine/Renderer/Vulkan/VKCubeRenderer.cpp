...
void VKCubeRenderer::prepare()
{
	...
	m_texture = createTexture("Resources/Textures/texture.png");
	...
	{
	  loadShaderModule("Resources/Shaders/cube.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
	  loadShaderModule("Resources/Shaders/cube.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
	};
...
void VKCubeRenderer::cleanup()
{
	for (auto& v : m_uniformBuffers)
	{
		vkDestroyBuffer(m_Context.m_Device->m_LogicalDevice, v.buffer, nullptr);
		vkFreeMemory(m_Context.m_Device->m_LogicalDevice, v.memory, nullptr);
	}
	vkDestroySampler(m_Context.m_Device->m_LogicalDevice, m_sampler, nullptr);
	vkDestroyImage(m_Context.m_Device->m_LogicalDevice, m_texture.image, nullptr);
	vkDestroyImageView(m_Context.m_Device->m_LogicalDevice, m_texture.view, nullptr);
	vkFreeMemory(m_Context.m_Device->m_LogicalDevice, m_texture.memory, nullptr);
	...
	vkDestroyDescriptorPool(m_Context.m_Device->m_LogicalDevice, m_descriptorPool, nullptr);
	vkDestroyDescriptorSetLayout(m_Context.m_Device->m_LogicalDevice, m_descriptorSetLayout, nullptr);
}

void VKCubeRenderer::makeCommand(VkCommandBuffer command)
{
	// ユニフォームバッファの中身を更新する.
	...
	shaderParam.mtxProj = perspective(glm::radians(60.0f), 1600.0f / 900.0f, 0.01f, 100.0f);
	{
		...
		void* p;
		vkMapMemory(m_Context.m_Device->m_LogicalDevice, memory, 0, VK_WHOLE_SIZE, 0, &p);
		memcpy(p, &shaderParam, sizeof(shaderParam));
		vkUnmapMemory(m_Context.m_Device->m_LogicalDevice, memory);
	}
...
void VKCubeRenderer::makeCubeGeometry()
{
	const float k = 1.0f;
...
void VKCubeRenderer::prepareUniformBuffers()
{
	m_uniformBuffers.resize(m_SwapChain.m_Images.size());
	for (auto& v : m_uniformBuffers)
	{
...
void VKCubeRenderer::prepareDescriptorSetLayout()
{
	...
	vkCreateDescriptorSetLayout(m_Context.m_Device->m_LogicalDevice, &ci, nullptr, &m_descriptorSetLayout);
}

void VKCubeRenderer::prepareDescriptorPool()
{
	...
	vkCreateDescriptorPool(m_Context.m_Device->m_LogicalDevice, &ci, nullptr, &m_descriptorPool);
}

void VKCubeRenderer::prepareDescriptorSet()
{
	...
	vkAllocateDescriptorSets(m_Context.m_Device->m_LogicalDevice, &ai, m_descriptorSet.data());
	...
	for (int i = 0; i<int(m_uniformBuffers.size()); ++i)
	{
		...
		vkUpdateDescriptorSets(m_Context.m_Device->m_LogicalDevice, uint32_t(writeSets.size()), writeSets.data(), 0, nullptr);
	}
}

BufferObject VKCubeRenderer::createBuffer(uint32_t size, VkBufferUsageFlags usage, VkMemoryPropertyFlags flags)
{
	BufferObject obj;
...
TextureObject VKCubeRenderer::createTexture(const char* fileName)
{
...

	{
		...
		vkCreateImage(m_Context.m_Device->m_LogicalDevice, &ci, nullptr, &texture.image);

		// メモリ量の算出
		VkMemoryRequirements reqs;
		vkGetImageMemoryRequirements(m_Context.m_Device->m_LogicalDevice, texture.image, &reqs);
		VkMemoryAllocateInfo info{};
		info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		info.allocationSize = reqs.size;
		// メモリタイプの判定
		...
		// メモリの確保
		vkAllocateMemory(m_Context.m_Device->m_LogicalDevice, &info, nullptr, &texture.memory);
		// メモリのバインド
		vkBindImageMemory(m_Context.m_Device->m_LogicalDevice, texture.image, texture.memory, 0);
	}

	{
		...
		void* p;
		vkMapMemory(m_Context.m_Device->m_LogicalDevice, stagingBuffer.memory, 0, VK_WHOLE_SIZE, 0, &p);
		memcpy(p, pImage, imageSize);
		vkUnmapMemory(m_Context.m_Device->m_LogicalDevice, stagingBuffer.memory);
	}
	...
	VkCommandBuffer command;
	{
		...
		vkAllocateCommandBuffers(m_Context.m_Device->m_LogicalDevice, &ai, &command);
	}
	...
	vkQueueSubmit(m_Context.m_Device->m_GraphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
	{
		// テクスチャ参照用のビューを生成
		...
		vkCreateImageView(m_Context.m_Device->m_LogicalDevice, &ci, nullptr, &texture.view);
	}

	vkDeviceWaitIdle(m_Context.m_Device->m_LogicalDevice);
	vkFreeCommandBuffers(m_Context.m_Device->m_LogicalDevice, m_Context.m_Device->m_CommandPool, 1, &command);

	// ステージングバッファ解放.
	vkFreeMemory(m_Context.m_Device->m_LogicalDevice, stagingBuffer.memory, nullptr);
	vkDestroyBuffer(m_Context.m_Device->m_LogicalDevice, stagingBuffer.buffer, nullptr);

	stbi_image_free(pImage);
	return texture;
}

void VKCubeRenderer::setImageMemoryBarrier(VkCommandBuffer command, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
VkSampler VKCubeRenderer::createSampler()
{
	VkSampler sampler;
	...
	vkCreateSampler(m_Context.m_Device->m_LogicalDevice, &ci, nullptr, &sampler);
	return sampler;
}

VkPipelineShaderStageCreateInfo VKCubeRenderer::loadShaderModule(const char* fileName, VkShaderStageFlagBits stage)
{
